  ...
  if (#clipboard_history == 0) then
    ...
    for k,v in pairs(clipboard_history) do
      if (string.len(v) > label_length) then
        table.insert(menuData,1, {title=string.sub(v,0,label_length).."…", shortcut=tostring(#clipboard_history - k + 1), fn = function() putOnPaste(v,key) end }) -- Truncate long strings
      else
        table.insert(menuData,1, {title=v, shortcut=tostring(#clipboard_history - k + 1), fn = function() putOnPaste(v,key) end})
      end -- end if else
    end-- end for
  end-- end if else
  -- footer
...
hs.hotkey.bind({"shift", "ctrl"}, "o", popupMenu)
hs.hotkey.bind({"shift", "ctrl", "option"}, "o", popupMenu)
